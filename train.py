...
def main(args):
    ...
    num_episodes = 40 ## total trajectories for training
    state_dim = 14
    lr_backbone = 1e-5
    backbone = 'resnet18'
    if policy_class == 'ACT':
...
def forward_pass(data, policy):
    ...
    image_data, qpos_data, action_data, is_pad = (image_data.cuda() , qpos_data.cuda(), action_data.cuda(), is_pad.cuda()) if torch.cuda.is_available() else image_data, qpos_data, action_data, is_pad
    task_emb = task_emb.cuda() if torch.cuda.is_available() else task_emb
