...
def main(args):
    ...
    task_emb = torch.from_numpy(task_emb).float().cuda() if torch.cuda.is_available() else torch.from_numpy(task_emb).float()
    ...
    policy.cuda() if torch.cuda.is_available() else None
    policy.eval()
    ...
    ### evaluation loop
    if temporal_agg:
        all_time_actions = torch.zeros([max_timesteps, max_timesteps+num_queries, state_dim]).cuda() if torch.cuda.is_available() else torch.zeros([max_timesteps, max_timesteps+num_queries, state_dim])

    qpos_history = torch.zeros((1, max_timesteps, state_dim)).cuda() if torch.cuda.is_available() else torch.zeros((1, max_timesteps, state_dim))
